...
class App extends Component {
	...
	componentDidMount() {
		axios
			.get('https://newsapi.org/v2/everything?q=technology&pageSize=100&apiKey=2297410c24494e6ca846b19e08376bd3')
			.then((res) => {
				console.log(res.data);
				const select = (gaa, a, b, c, d) => {
					gaa[a + c].type = 'wide';
					gaa.slice(a, b).map((e) => {
						return (e.label = e.author);
					});
				...
				const setter = (res, a) => {
					res[a].label = null;
					return [
						select(res, a, a + 4, 0, 3),
						select(res, a + 4, a + 8, 1, 0),
						select(res, a + 8, a + 12, 3, 2),
						select(res, a + 12, a + 18, 2, 0)
					];
				};
	...
	render() {
		return (
			<div>
				<MainHead item={this.state.navigationItems} show={this.state.show} showNav={this.showNav} />
				<News articles={this.state.articles[0]} />
				{/* <img alt='das' width='50%' height='150px' className='center' src='https://www.looofa.com/images/ad_web.png'/> */}
				<News articles={this.state.articles[1]} />
				<News articles={this.state.articles[2]} />
				<Collage articles={this.state.articles[3]} />
			</div>
		);
	}
}
